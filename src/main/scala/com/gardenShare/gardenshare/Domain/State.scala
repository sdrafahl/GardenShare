...
object State {
  private lazy val stateDecoder: Decoder[State] =
    Decoder.decodeString.emap(decoder(_))

  private lazy val stateEncoder: Encoder[State] =
    Encoder.encodeString.contramap(encodeState)

  implicit lazy val stateCodec: Codec[State] = Codec.from(stateDecoder, stateEncoder)

  private def decoder(st: String): Either[String, State] = st match {
    case "AL" => Right(AL)
    case "AK" => Right(AK)
    case "AZ" => Right(AZ)
    case "AR" => Right(AR)
  ...
  private def encodeState(x: State) = x match {
    case AL => "AL"
    case AK => "AK"
    case AZ => "AZ"
    case AR => "AR"
